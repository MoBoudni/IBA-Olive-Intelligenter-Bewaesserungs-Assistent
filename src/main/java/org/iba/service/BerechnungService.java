...
public class BerechnungService {
    ...
    public Map<String, Double> berechneGesamtWasserbedarfProParzelle() {
        ...
        for (Parzelle parzelle : alleParzellen) {
            // 3. Wetterdaten für die Parzelle abrufen (Wir nehmen an, Messungen sind Parzellen-spezifisch)
            Wetterdaten aktuelleWetterdaten = messwerteRepository.findeLetzteMessung(parzelle.getParzelleId());

            if (aktuelleWetterdaten == null) {
                System.err.println("WARNUNG: Keine Wetterdaten für Parzelle " + parzelle.getName() + " gefunden. Überspringe Berechnung.");
            ...
            double gesamtbedarf = berechneWasserbedarf(baeumeAufParzelle, aktuelleWetterdaten);
     ...
     * @param wetterdaten Die aktuellen Wetterdaten.
     * @return Der Gesamtbedarf in Litern.
     */
    private double berechneWasserbedarf(List<Baum> baeume, Wetterdaten wetterdaten) {
        if (baeume.isEmpty()) {
            return 0.0;
        }

        double temperatur = wetterdaten.getTemperatur();
        double niederschlag = wetterdaten.getNiederschlag();
        ...
        // 2. Niederschlagsfaktor: Wenn Niederschlag > 5mm, Bewässerung um 50% reduzieren.
